...
// ══════════════════════════════════════════════════════
// VIDEOS — Agrega tus videos aquí
// Copia el bloque que genera el panel admin y pégalo aquí
// ══════════════════════════════════════════════════════
const BASE_VIDEOS = [
    // Pega aquí los videos generados por el panel admin:
    // { id:1, title:'Mi video', category:'youtube', ytId:'XXXXXXXXXXX', description:'Descripción.' },
];
...
function renderClients() {
    ...
    // Duplicar lista para el loop infinito (necesita al menos 2x para que sea continuo)
    const list = clients.length ? clients : DEFAULT_CLIENTS;
    const repeated = [...list, ...list]; // duplicado para loop
    el.innerHTML = repeated.map(makeCard).join('');

    // Ajustar duración según cantidad de clientes (más clientes = más lento)
...
function showAdminTab(name) {
    ...
    if (name === 'manage')       { renderAdminList(); updateCodeBlock(); }
...
function renderAdminList() {
    ...
    el.querySelectorAll('.btn-del').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (idx!==-1) { videos.splice(idx,1); saveVideos(); refreshAll(); renderAdminList(); updateCodeBlock(); }
        });
    });
}
